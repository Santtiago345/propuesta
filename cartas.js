var Cartas = (function () {
  ...
  var currentCat = -1; // -1 = vista principal
  var currentIndex = -1;
  var list = [];
  ...
  function renderizarGrid() {
    ...
    if (currentCat === -1) {
      // Vista principal: cartas directas + carpeta de nostalgia
      cartasDirectas.forEach(function (carta) {
        var wrapper = document.createElement('div');
        wrapper.className = 'carta-wrapper';
        wrapper.onclick = function () { list = cartasDirectas; abrirOverlay(0); };
        wrapper.innerHTML =
          '<div class="carta">' +
      ...
      folder.className = 'carta-wrapper folder-wrapper';
      folder.onclick = function () { abrirCategoria(0); };
      folder.innerHTML =
        '<div class="carta folder-card">' +
          '<div class="carta-front folder-front">' +
            '<div class="carta-seal">\uD83D\uDCC1</div>' +
            '<h2 class="carta-name">Cartas de nostalgia</h2>' +
            '<p class="carta-hint">' + nostalgiaCartas.length + ' carta' + (nostalgiaCartas.length !== 1 ? 's' : '') + '</p>' +
          '</div>' +
        '</div>';
      ...
      list = nostalgiaCartas;
      list.forEach(function (carta, i) {
        var wrapper = document.createElement('div');
        wrapper.className = 'carta-wrapper';
        wrapper.onclick = function () { abrirOverlay(i); };
        wrapper.innerHTML =
          '<div class="carta">' +
            '<div class="carta-front">' +
              '<div class="carta-seal">\uD83D\uDCDD</div>' +
  ...
  function anterior() {
    if (currentIndex > 0) abrirOverlay(currentIndex - 1);
  }

  function siguiente() {
    if (currentIndex < list.length - 1) abrirOverlay(currentIndex + 1);
  }
